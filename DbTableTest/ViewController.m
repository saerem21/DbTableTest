...
//    
//    int ret = sqlite3_open([dbFilePath UTF8String], &db);
//   //NSAssert1(SQLITE_OK == ret, @"err on opening db %s", sqlite3_errmsg(db));
//    NSLog(@"success");
//    
//    if(NO == existFile){
...
//    
//    if(SQLITE_OK != ret){
//        NSLog(@"err on insert New Data %s",errMsg);
//    }
...
//- (void)resolveData{
//    [data removeAllObjects];
//    NSString *qeuryStr = @"SELECT rowid, title FROM MOVIE";
//    sqlite3_stmt *stmt;
//    int ret = sqlite3_prepare_v2(db, [qeuryStr UTF8String], -1, &stmt, NULL);
//    
//  // NSAssert2(SQLITE_OK == ret, @"%d err %s", ret, sqlite3_errmsg(db));
//    
//    while (SQLITE_ROW == sqlite3_step(stmt)) {
//        int rowID = sqlite3_column_int(stmt, 0);
//        char *title = (char *)sqlite3_column_text(stmt, 1);
...
//        
//        if(SQLITE_OK != ret){
//            NSLog(@"err (%d) on deleting data:%s",ret,errorMsg);
//            
//        }
//        [self resolveData];
//    }
